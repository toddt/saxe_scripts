...
for i=1:nbchunks
    ...
    % Add code from Hilary to interp art timepoints before filtering
                fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...interp over art');
            
            %-ADDED by HR: linearly interpolate over artifact timepoints.
             numRuns = str2num(SPM.xsDes.Number_of_sessions);
             art_reg = cell(1,1);
             art_tps = cell(1,numRuns);
             all_tps = [1:size(SPM.xY.VY,1)]';            
             tpsPerRun = nScan/numRuns;

             for r=1:numRuns
                 %art_tmp = zeros(size(SPM.Sess(r).C.C,2),1); this assumes
                 %only artifact timepoint regs (no cc, no mot)
                 count_art = 0;
                 count_cc = 0;
                 
                 %NOTE- lines 690 and 692 need to be fixed. Currently all
                 %cc and art regs 
                 %recognized as cc regs... 
                 %regexp(behavs,'\.','split')
                 
                 for v = 1:size(Vbeta,2)
                     betastring = regexp(Vbeta(v).descrip,['run_' num2str(r)],'match');
                     if  ~isempty(betastring)          %~isempty(regexp(Vbeta(v).descrip,['\w*cc*\w*run_' num2str(r)])) %any(strfind(char(Vbeta(v).descrip),'cc'));
                         if any(strfind(char(Vbeta(v).descrip),'cc'))
                            count_cc = count_cc + 1;
                         elseif any(strfind(char(Vbeta(v).descrip),'artifact'));
                            %elseif ~isempty(regexp(Vbeta(v).descrip,['\w*artifact*\w*run_' num2str(r)])) %any(strfind(char(Vbeta(v).descrip),'artifact'));
                            count_art = count_art + 1;       
                         end
                     end
                 end
                     
                 art_tmp = zeros(count_art,1);

                for art = 1:size(art_tmp,1)
                    art_tmp(art) = find(SPM.Sess(r).C.C(:,art));
                end
                art_tps{r} = art_tmp;
                ...
                if r > 1
                    all_arts = [all_arts; art_tps{r}+((r-1)*tpsPerRun)];
                end
            ...
            if size(a,1) > 0
                leftover_tmp = zeros(size(p,1),1);
                for i=1:length(a) %for each art TP
                    p = [p(1:a(i)-1,:); zeros(1,CrS); p(a(i)+1:end,:)]; %find artTP row, fill with zero (temporary)
                    leftover_tmp(a) = 1;
                end
                %Y = p'; %check

                %leftover = find(all(p==0,2)); %all chunks. but chunks
                %aren't all next to eachother. commented bc modeling script
                %goes by planes, and has more rows that are all zeros for
                %planes w/ few voxels.
                leftover = find(leftover_tmp); %
                ...
                use_tps = setdiff(all_tps,a);

                for i=1:length(starts)
                    for reg = 1:size(Y,2) %reg = vox
                        %set beg tp (tp before art, or 0)
                        if a(starts(i)) == 1 %if first tp is artreg
                            %beg = p(a(starts(i)),reg); %set to 0
                             beg = mean(Y(use_tps,reg)); %set to mean value for vox (from used tps in raw tc)
                        else
                        ...
                        if a(starts(i))+spots(i) == all_tps(end)+1
                            ...
                            last = mean(Y(use_tps,reg)); %set to mean value for vox (from used tps in raw tc)
                        else
                        ...
                        if a(starts(i)) == 1
                            ...
                            p(starts(i):spots(i)+1,reg) = interp1([1;spots(i)+1],[beg;last],[1:spots(i)+1],'nearest')';
                        elseif a(starts(i))+spots(i) == all_tps(end)+1
                            %p(a(starts(i))-1:a(starts(i))+spots(i)-1,reg) = linspace(beg,last,spots(i)+1)';
                            p(a(starts(i))-1:a(starts(i))+spots(i)-1,reg) = interp1([1;spots(i)+1],[beg;last],[1:spots(i)+1],'nearest')';
                        else
